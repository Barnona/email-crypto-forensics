...
def _build_json_bytes(sessions: list[EmailSession]) -> bytes:
    ...
    data = {
        ...
        "sessions": [json.loads(json.dumps(session, default=_json_default)) for session in ordered],
    }
...
def _render_downloads(sessions: list[EmailSession]) -> None:
    ...
    cols[0].download_button(
        "Download JSON", data=_build_json_bytes(sessions),
        file_name="securemailscope-report.json", mime="application/json", width="stretch"
    )
    cols[1].download_button(
        "Download HTML", data=render_html(_sort_sessions(sessions)).encode("utf-8"),
        file_name="securemailscope-report.html", mime="text/html", width="stretch"
    )
    with tempfile.TemporaryDirectory(prefix="securemailscope-report-") as tmp:
        ...
        try:
            ...
            cols[2].download_button(
                "Download PDF", data=pdf_path.read_bytes(),
                file_name="securemailscope-report.pdf", mime="application/pdf", width="stretch"
            )
...
def _overview(sessions: list[EmailSession]) -> None:
    ...
    no_tls_observed = sum(not s.tls_attempted and s.tls_session is None for s in sessions)
    ...
    st.caption(
        f"TLS state: {encrypted} encrypted/observed · {no_tls_observed} with no TLS handshake observed · "
    ...
    severity_df = pd.DataFrame(
        {"Sessions": [counts[s.value] for s in Severity]}, index=[s.value for s in Severity]
    )
...
def _session_explorer(sessions: list[EmailSession]) -> None:
    ...
    protocols = c1.multiselect("Protocol", sorted(df["Protocol"].unique()), default=sorted(df["Protocol"].unique()))
    severities = c2.multiselect("Severity", [s.value for s in Severity], default=[s.value for s in Severity])
    tls_states = c3.multiselect("TLS state", sorted(df["TLS state"].unique()), default=sorted(df["TLS state"].unique()))
    filtered = df[
        df["Protocol"].isin(protocols) & df["Severity"].isin(severities) & df["TLS state"].isin(tls_states)
    ]
    ...
    session_map = {s.session_id: s for s in _sort_sessions(sessions) if s.session_id in visible_ids}
    selected_id = st.selectbox(
        "Inspect session", list(session_map),
    ...
    with left:
        ...
        st.write(f"**STARTTLS:** {'used' if session.starttls_used else 'offered' if session.starttls_offered else 'not detected'}")
    with right:
...
def _findings(sessions: list[EmailSession]) -> None:
    findings = [(session, finding) for session in _sort_sessions(sessions) for finding in session.findings]
    if not findings:
        st.success("No findings were generated for the analysed sessions.")
        return
    rows = [
        {
            "Severity": finding.severity.value, "Rule": finding.rule_id, "Category": finding.category,
            "Session": session.session_id, "Description": finding.description, "Source": finding.source,
        }
        for session, finding in findings
    ]
    df = pd.DataFrame(rows)
    severity_filter = st.multiselect("Finding severity", [s.value for s in Severity], default=[s.value for s in Severity])
    filtered = df[df["Severity"].isin(severity_filter)]
    st.dataframe(filtered, width="stretch", hide_index=True)
    if filtered.empty:
        st.info("No findings match the selected severity filters.")
        return
    selected_finding_ids = set(zip(filtered["Session"], filtered["Rule"], filtered["Description"]))
    for session, finding in findings:
        if (session.session_id, finding.rule_id, finding.description) not in selected_finding_ids:
            continue
...
def _tls_analysis(sessions: list[EmailSession]) -> None:
    ...
    with left:
        ...
        st.dataframe(pd.DataFrame.from_dict(versions, orient="index", columns=["Sessions"]), width="stretch")
    with right:
        st.markdown("**Cipher suites**")
        st.dataframe(pd.DataFrame.from_dict(ciphers, orient="index", columns=["Sessions"]), width="stretch")
    ...
    for session in tls_sessions:
        for cert in session.tls_session.certificates:
            cert_rows.append(
                {
                    "Session": session.session_id, "Subject": cert.subject, "Issuer": cert.issuer,
                    "Key": f"{cert.public_key_algorithm} / {cert.key_size_bits} bits",
                    "Signature": cert.signature_algorithm, "Expired": cert.is_expired,
...
def _timeline(sessions: list[EmailSession]) -> None:
    rows = [
        {
            "Start": s.start_time, "End": s.end_time or s.start_time, "Session": s.session_id,
            "Protocol": s.protocol.value, "Severity": overall_severity(s).value,
            "Risk score": s.risk_score, "TLS state": _tls_state(s),
            "Capture": "Complete" if s.capture_complete else "Incomplete",
        }
...
def _analyse_uncached(
    pcap_bytes: bytes, risk_model: str | None, anomaly_model: str | None, contamination: float
) -> list[EmailSession]:
    with tempfile.TemporaryDirectory(prefix="securemailscope-ui-") as tmp:
        ...
        return analyze(path, risk_path, anomaly_path, contamination=contamination)


@st.cache_data(show_spinner=False)
def _analyse_cached(
    pcap_hash: str, pcap_bytes: bytes, risk_model: str | None, anomaly_model: str | None, contamination: float
) -> list[EmailSession]:
    """Cache analysis by PCAP content hash and analysis configuration."""
    # The hash is an explicit cache key so two identical uploads share one result
    # within the current Streamlit server while retaining the original bytes as data.
    _ = pcap_hash
    return _analyse_uncached(pcap_bytes, risk_model, anomaly_model, contamination)
...
def main() -> None:
    ...
    if not uploaded:
        ...
        return
    pcap_bytes = uploaded.getvalue()
    pcap_hash = hashlib.sha256(pcap_bytes).hexdigest()
    try:
        with st.spinner("Reassembling streams and analysing TLS posture…"):
            sessions = _analyse_cached(pcap_hash, pcap_bytes, risk_model or None, anomaly_model or None, contamination)
    ...
    tabs = st.tabs(["Security Posture", "Session Explorer", "Risk Findings", "TLS Analysis", "Timeline", "Reports"])
    with tabs[0]:
        _overview(sessions)
    with tabs[1]:
