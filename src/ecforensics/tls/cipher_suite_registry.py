"""
IANA TLS Cipher Suite Registry -- codepoint to canonical name mapping.

Source: tshark's own value_string table (tshark -G values, filtered to
tls.handshake.ciphersuite), which tracks the IANA TLS Cipher Suites registry.
This closes the TODO in tls/cipher_suites.py that asked for "the full IANA
TLS Cipher Suite Registry" -- handshake_parser.py needs this because tshark's
raw `-T fields` output gives the cipher suite as a hex codepoint (e.g.
"0xc030"), not the canonical name (e.g. "TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384")
that risk_engine/cipher_suites.py's pattern matching expects.

Regenerate with:
    tshark -G values | awk -F'\t' \
        '$1=="V" && $2=="tls.handshake.ciphersuite" \
        {printf "    0x%04x: \"%s\",\n", $3, $4}'
"""
