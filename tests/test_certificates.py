...
_PCAP_DIR = _ROOT / "data" / "pcaps"
_CERT_DIR = _ROOT / "data" / "certs"
_HAS_TSHARK = shutil.which("tshark") is not None


@pytest.mark.skipif(not (_HAS_TSHARK and (_PCAP_DIR / "tls_handshake.pcap").exists()), reason="real TLS fixture unavailable")
...
@pytest.mark.skipif(not (_HAS_TSHARK and (_PCAP_DIR / "expired_handshake.pcap").exists()), reason="expired TLS fixture unavailable")
def test_extract_and_parse_expired_cert():
    der = extract_der_certificates_from_pcap(_PCAP_DIR / "expired_handshake.pcap", "0")
    assert der
    cert = parse_certificate(der[0])
...
@pytest.mark.skipif(not (_HAS_TSHARK and (_PCAP_DIR / "tls13_handshake.pcap").exists()), reason="TLS 1.3 fixture unavailable")
